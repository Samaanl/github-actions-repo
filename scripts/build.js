/**
 * Build step for the demo app.
 *
 * 1. Copies `public/` into `dist/`.
 * 2. Copies the shared `src/grades.js` module in beside it, so the browser and
 *    the unit tests run byte-identical logic.
 * 3. Writes `dist/build-info.json` from the environment variables GitHub
 *    Actions injects into every job. That file is what the page displays as
 *    "build provenance" - it is the visible proof that CI produced this build.
 *
