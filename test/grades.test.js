...
describe('isPass', () => {
  ...
  // The two below hard-code 40 rather than deriving it from PASS_MARK.
  // That is deliberate: a test written in terms of the constant it is meant to
  // protect cannot detect a change to that constant. These pin the *business
  // rule* ("the university pass mark is 40"), not the implementation.
...
describe('calculateSGPA', () => {
  test('averages equally weighted subjects', () => {
    const subjects = [
      { credits: 4, marks: 95 }, // 10 points
      { credits: 4, marks: 75 }, // 8 points
    ];
  ...
  test('respects credit weighting', () => {
    const subjects = [
      { credits: 6, marks: 95 }, // 10 points x 6 = 60
      { credits: 2, marks: 45 }, // 5 points x 2 = 10
    ];
    // (60 + 10) / 8 = 8.75
  ...
  test('rounds to two decimal places', () => {
    const subjects = [
      { credits: 3, marks: 95 }, // 10
      { credits: 3, marks: 65 }, // 7
      { credits: 3, marks: 55 }, // 6
    ];
