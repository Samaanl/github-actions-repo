/**
 * Core business logic for the Student Result & SGPA Calculator.
 *
 * This file is deliberately dependency-free so that the exact same module
 * runs in Node.js (for the unit tests in CI) and in the browser (for the
 * page we deploy to GitHub Pages).
 */

/** University 10-point grading scale. Ordered highest threshold first. */
...
/**
 * Maps raw marks to a grade band.
 * @param {number} marks
...
/**
 * @param {number} marks
 * @returns {boolean} true when the student has cleared the subject
 */
...
/**
 * Weighted grade-point average across subjects.
 * @param {Array<{ name?: string, credits: number, marks: number }>} subjects
...
/**
 * Full result sheet for one student.
 * @param {Array<{ name?: string, credits: number, marks: number }>} subjects
 */
...
/**
 * Human-readable division based on SGPA.
 * @param {number} sgpa
 * @param {number} backlogCount
 */
